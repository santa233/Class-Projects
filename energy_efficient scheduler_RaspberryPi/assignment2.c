...
void learn_workloads(SharedVariable* sv) {
	...
	thread_button(sv);
        long long curTime_after = get_current_time_us();
	printDBG("exe_time_button=%u ",(curTime_after-curTime_before));
}
...
TaskSelection select_task(SharedVariable* sv, const int* aliveTasks, long long idleTime) {
	...
	long long next_deadline[NUM_TASKS],curr_time,ED;
	//while()
	...
	{
		if(aliveTasks[i]==1)
			next_deadline[i]=curr_time+workloadDeadlines[i];
		if(ED>next_deadline[i])
		{
			ED=next_deadline[i];
			min=i;
		}
